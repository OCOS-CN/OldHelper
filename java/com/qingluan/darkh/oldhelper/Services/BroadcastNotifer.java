...
public class BroadcastNotifer {
    ...
    public boolean sendIntent(String action,String jsonData){
        ...
        intent.putExtras(data_bundle);
        this.localcontext.sendBroadcast(intent);
        Log.d(tag, "send intent to broadcast from " + this.localcontext.toString()+" to " +action);
        return true;
    }
    ...
    public boolean sendIntentService(Context context,Bundle info){
        Log.d(tag, "starting send intent to Talkingserver ");
        Intent intent = new Intent(context,TalkService.class);
